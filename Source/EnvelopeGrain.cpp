...
void EnvelopeGrain::paint(juce::Graphics& g) {
  ...
  juce::PathStrokeType pathStroke =
      juce::PathStrokeType(2, juce::PathStrokeType::JointStyle::mitered, juce::PathStrokeType::EndCapStyle::rounded);
  ...
  while (curXStart < getWidth()) {
    ...
    envPath.lineTo(juce::Point<float>(juce::jmax(curXStart, curXStart + (mTilt * envWidth) - shapeWidth), envTop));
    envPath.lineTo(juce::Point<float>(juce::jmin(curXStart + envWidth, curXStart + (mTilt * envWidth) + shapeWidth), envTop));
