...
void EnvelopeADSR::paint(juce::Graphics& g) {
  ...
  g.drawLine(
      juce::Line<float>(1.0f, getHeight() - 1.0f,
                        mAttack * getWidth() * 0.375f + 1.0f, envTop),
      highlightWidth);
  g.setColour(mIsActive ? envColour.brighter().brighter()
                        : juce::Colours::darkgrey);
  g.drawLine(juce::Line<float>(mAttack * getWidth() * 0.375f, envTop,
                               mAttack * getWidth() * 0.375f +
                                   mDecay * getWidth() * 0.375f + 1.0f,
                               sustainY),
             highlightWidth);
  ...
  g.drawLine(juce::Line<float>(
                 ...
                 sustainY, getWidth() * 0.75f + 1.0f, sustainY),
             highlightWidth);
  g.setColour(mIsActive ? envColour.brighter().brighter().brighter().brighter()
                        : juce::Colours::darkgrey);
  g.drawLine(
      juce::Line<float>(getWidth() * 0.75f, sustainY,
